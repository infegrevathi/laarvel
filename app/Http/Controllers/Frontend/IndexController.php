...
class IndexController extends Controller
{
    ...
    public function ProductDetails($id,$slug){
		...
		$product_details = Product::with('category')->findOrFail($id);
    ...
    public function CategoryDetails(Request $request,$id){
		$category_details = Product::where('status',1)->where('category_id',$id)->orderBy('id','DESC')->paginate(6);
		$color = Colors::get();
        //  Load More Product with Ajax 
		if ($request->ajax()) {
        ...
        //  End Load More Product with Ajax 
	 	...
	 	return view('frontend.product.category_details',compact('category_details','color'));
